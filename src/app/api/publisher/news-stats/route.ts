...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: statusCounts, error } = await client
      .rpc('get_publisher_news_stats', { p_publisher_id: publisherId })
    ...
    if (error) {
      ...
      while (hasMore) {
        const { data, error: fetchError } = await client
          .from('ai_news')
          .select('status')
          .eq('author_id', publisherId)
          .range(offset, offset + batchSize - 1)

        if (fetchError || !data || data.length === 0) {
          hasMore = false
        } else {
          allStatuses.push(...data.map(n => n.status))
    ...
    const stats = {
      ...
      draft: (statusCounts as any)?.draft || 0,
    }
    ...
    console.error('获取资讯统计数据错误:', error)
    return NextResponse.json(
