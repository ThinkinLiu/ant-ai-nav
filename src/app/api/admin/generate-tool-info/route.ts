  ...
  try {
    ...
    try {
      ...
      if (searchResponse.web_items && searchResponse.web_items.length > 0) {
        ...
        const relevantItems = searchResponse.web_items.filter(item => {
...
你需要返回以下信息（以JSON格式）：
{
  ...
  "long_description": "详细介绍（200-400字，包含主要功能、使用场景、特点等，必须基于搜索结果）",
...
请严格按照搜索结果中的信息生成JSON格式的工具详情，不要编造任何信息。`
    ...
    try {
      ...
      return NextResponse.json({
        success: false,
        error: '信息解析失败，请重试或手动填写工具信息'
      })
    }

    // 验证生成的信息是否合理（检查是否包含"暂无信息"等占位符过多）
    const placeholderCount = [result.description, result.long_description, result.pricing_info]
    ...
    if (placeholderCount >= 2) {
      return NextResponse.json({
        success: false,
        error: `搜索到的信息不足以生成完整的工具介绍。建议手动填写工具信息。\n\n已搜索到的片段：\n${searchResults.substring(0, 500)}...`
      })
    }
